...
from codes.utils import get_logger, NASCTR
...
class Dataset(data.Dataset):
    ...
    def _read(self):
        ...
        self._num_data = self._num_train_data + self._num_test_data
    ...
    def _check_preprocess_file(self):
        return os.path.exists(os.path.join(self._save_path, DENSE_TRAIN)) and \
               ...
               os.path.exists(os.path.join(self._save_path, LABEL_TEST))
...
class PreprocessedDataset(data.Dataset):
    ...
    def get_search_dataloader(self, train_ratio, valid_ratio, batch_size, num_workers=8, random_val=True):
        train_num = int(self._num_data * train_ratio)
        valid_num = int(self._num_data * valid_ratio)
        test_num = self._num_data - train_num - valid_num
    ...
    def get_eval_dataloader(self, train_ratio, valid_ratio, batch_size, num_workers=4, shuffle=True):
        ...
        indices = list(range(self._num_data))
        if shuffle:
            np.random.shuffle(indices)
        train_sampler = torch.utils.data.sampler.SubsetRandomSampler(indices[:train_num])
        val_sampler = torch.utils.data.sampler.SubsetRandomSampler(indices[train_num:train_num + valid_num])
        test_sampler = torch.utils.data.sampler.SubsetRandomSampler(indices[train_num + valid_num:])
        ...
        train_dataloader = DataLoaderX(self, batch_size=batch_size, num_workers=num_workers, pin_memory=True, sampler=train_sampler)
        val_dataloader = DataLoaderX(self, batch_size=batch_size, num_workers=num_workers, pin_memory=True, sampler=val_sampler)
        test_dataloader = DataLoaderX(self, batch_size=batch_size, num_workers=num_workers, pin_memory=True, sampler=test_sampler)
    ...
    def _read(self):
        if not self._repreprocess:
            if self._check_preprocess_file():
                self._logger.info("Use preprocessed dataset")
                self._dense_feats = np.load(os.path.join(self._save_path, PRE_DENSE_FEATES_FILE))
                self._sparse_feats = np.load(os.path.join(self._save_path, PRE_SPARSE_FEATES_FILE))
                self._labels = np.load(os.path.join(self._save_path, LABELS_FEATES_FILE))
                self._field_dims = np.load(os.path.join(self._save_path, FIELD_DIMS_FILE))
            else:
                self._repreprocess = True
            if os.path.exists(os.path.join(self._save_path, PRETRAINED_EMBEDDINGS_DIR, BEST_PRETRAINED_EMBEDDINGS_FILE)) and self._use_pretrained_embeddings:
                self._pretrained_embeddings = torch.load(os.path.join(self._save_path, PRETRAINED_EMBEDDINGS_DIR, BEST_PRETRAINED_EMBEDDINGS_FILE))
            else:
                self._use_pretrained_embeddings = False
