...
from codes.architectures import Architecture
from codes.blocks.block import BLOCKS
from codes.params import init_block2params, get_num_params, set_num_params, get_num_params_info
from codes.utils import NASCTR, get_logger


class MixedBlock(nn.Module):

    def __init__(self, block_params, node2type, block_keys,
                 num_inputs, block_id, num_init_node):
    ...
    def forward(self, prev_nodes, weights):
        ...
        if self._num_inputs == 2:
            chosen_prev_node2 = weights_list[num_prev_nodes:num_prev_nodes * 2].index(
                1)
        inputs = []
        x1 = sum([w * input if w.data > 0 else w for input,
                  w in zip(prev_nodes, weights[:num_prev_nodes])])
        node_type1 = self._node2type[chosen_prev_node1]
        inputs.append((x1, node_type1))
        if self._num_inputs == 2:
            x2 = sum([w * input if w.data > 0 else w for input,
                      w in zip(prev_nodes,
                               weights[num_prev_nodes:num_prev_nodes * 2])])
...
class Network(nn.Module):
    def __init__(self, dataset, args, logger=None):
        ...
        self._sparse_offsets = np.array(
            (0, *np.cumsum(self._field_dims)[self._num_dense_feats:-1]), dtype=np.long)
        ...
        self._block_out_dim = args.block_out_dim
        ...
        if args.use_pretrained_embeddings and dataset.pretrained_embeddings is not None:
            ...
            self._embedding_table = nn.Embedding(
                sum(self._field_dims), self._embedding_dim)
    ...
    def compute_loss(self, predicts, labels, regs,
                     use_reg=True, use_arch_loss=True):
        raise NotImplementedError()


class FixedNetwork(Network):
    def __init__(self, dataset, args, criterion, arch, logger=None):
        ...
        self._classifier = nn.Linear(
            len(self._concat_blocks_idxes) * self._block_out_dim, 1)
    ...
    def forward(self, input):
        ...
        if self._num_dense_feats > 0:
            ...
            dense_emb = self._embedding_table(
                raw_dense.new_tensor(dense_indexes))
            dense_emb = dense_emb * raw_dense.unsqueeze(-1)
            nodes.append(dense_emb)
            norms.append(torch.norm(dense_emb))
        if self._num_sparse_feats > 0:
            sparse_indexes = raw_sparse + \
                raw_sparse.new_tensor(self._sparse_offsets).unsqueeze(0)
        ...
        for i, block in enumerate(self._blocks):
            if i == 0 and self._num_init_node == 1:
                inputs = [(nodes[self._block_inputs[i][0]],
                           self._node2type[self._block_inputs[i][0]])]
            else:
            ...
            node = block(inputs)
            nodes.append(node)
        output = torch.cat([nodes[i]
                            for i in self._concat_blocks_idxes], dim=1)
    ...
    def compute_loss(self, predicts, labels, regs,
                     use_reg=True, use_arch_loss=False):
        labels = labels.float()
...
class DynamicNetwork(Network):
    def __init__(self, dataset, args, criterion, logger=None):
        ...
        self._logger.info(
            f"The parameter amount of each block: {get_num_params_info()}")

    def new(self):
        model_new = DynamicNetwork(
            self._dataset,
            self._args,
            self._criterion,
            self._logger).cuda()
    ...
    def forward(self, input):
        ...
        for i, block in enumerate(self._blocks):
            ...
            for j in range(self._get_num_input_node(i)):
                assert weights[j * self._num_prev_nodes[i]
                    : (j + 1) * self._num_prev_nodes[i]].sum() == 1
            assert weights[-self._num_blocks_type:].sum() == 1
            node = block(nodes, weights)
            nodes.append(node)
    ...
    def step(self, dense_valid, sparse_valid, labels_valid, arch_optimizer):
        self.zero_grad()
    ...
    def _arch_loss(self):
        ...
        regs = [torch.pow(self._arch_params[i][-self._num_blocks_type:], 2)
                * normal_p for i in range(self._num_blocks)]
    ...
    def _initialize(self):
        ...
        for i in range(self._num_blocks):
            alphas = torch.ones(self._get_num_input_node(
                i) * self._num_prev_nodes[i] + self._num_blocks_type) / 2
        ...
        for i in range(self._num_blocks):
            mixedBlock = MixedBlock(self._block_params, self._node2type, self._blocks_keys,
                                    num_inputs=self._get_num_input_node(i), block_id=i, num_init_node=self._num_init_node)
    ...
    def binarize(self, e_greedy=0):
        ...
        block_types = []
        prev = [
            np.array([j * self._num_prev_nodes[i] for j in range(self._get_num_input_node(i))])
            for i in range(self._num_blocks)
        ]
        if np.random.rand() <= e_greedy:
            for i in range(self._num_blocks):
                nodes = prev[i] + np.random.choice(
                    self._num_init_node + i, self._get_num_input_node(i), replace=False)
            ...
            for i in range(self._num_free_block):
                weights = self._arch_params[i]
                inputs = [weights[j *
                                  self._num_prev_nodes[i]:(j +
                                                           1) *
                                  self._num_prev_nodes[i]] for j in range(self._get_num_input_node(i))]
                ...
                nodes = prev[i] + nodes
                input_nodes.append(nodes)
                block_types.append(self._get_num_input_node(
                    i) * self._num_prev_nodes[i] + weights[-self._num_blocks_type:].argmax())
            ...
            for i in range(self._num_free_block, self._num_blocks):
                ...
                for input in inputs:
                    ...
                    while True:
                        ...
                        node_type = self._node2type[node]
                        if not (
                                node_type < 3 and input[node] not in selected_logits):
                            break
                    nodes.append(node)
                nodes = prev[i] + nodes
                input_nodes.append(nodes)
    ...
    def get_arch(self, use_softmax=True):
        def _parse(weights, primitives):
            ...
            for i in range(self._num_blocks):
                ...
                inputs_p = []
                for j in range(self._get_num_input_node(i)):
                    input_p = weight[j * self._num_prev_nodes[i]
                        : (j + 1) * self._num_prev_nodes[i]]
                    node = next(
                        filter(
                ...
                block_type = primitives[np.argmax(
                    weight[-self._num_blocks_type:])]
        ...
        arch, arch_p, params = _parse(
            self._arch_params, list(
                self._blocks_keys))
